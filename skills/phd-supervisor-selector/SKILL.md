...
- **禁止**写"匹配度最高"、"匹配面最广"、"很契合"、"直接对话"、"很有交集"——这是机器打分语言，不是人话
...
第三段可选——仅当满足「可以备选一下～」的触发条件时，才在第二段末尾加一句 `你可以备选一下～`。

**禁止事项**：
- 禁止使用任何 emoji 符号或图标（如 🔥👍⚠️ 等）
- 禁止使用主观评价用语如 `高度相关`、`很匹配`、`完美匹配`、`匹配度最高`、`匹配面最广`、`很契合`、`直接对话`、`很有交集`、`能搭上`
- 描述事实，不做评判
...
**触发条件**：导师的研究内容**部分满足**学生方向，但**同时有偏离**，值得让学生了解考虑。

**正确写法**：
- `教授；做心血管药物流行病学，但也有老年用药安全的研究方向，你可以备选一下～`
- `副教授；做药物经济学和卫生技术评估，也有糖尿病慢病管理方向，你可以备选一下～`
...
6. **读一遍**：像不像一个真实的人写给申请学生的备注？不是像 AI 审计报告？
...
| 备注中 SPA 标注和匹配标识混在同一段 | `Macquarie为SPA系统需浏览器验证；可以备选一下呢～` | SPA 标注放在第二段末尾；现匹配标识已废除，备注只保留职称+研究方向两段 |
